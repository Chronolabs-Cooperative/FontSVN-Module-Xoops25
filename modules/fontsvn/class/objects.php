 ...
 */
class fontsvnXoopsObject extends XoopsObject
{
	/**
	 * (non-PHPdoc)
	...
	function assignVar($key, $value)
	{
		if ($this->vars[$key]['data_type'] == XOBJ_DTYPE_OTHER) {
			parent::assignVar($key, fontsvnDecompressData($value));
		} elseif (strpos($key, 'pass')||strpos($key, 'password')) {
			parent::assignVar($key, fontsvnDecryptPassword($value, PLEASE_SALT . PLEASE_SALT_WHENSET));
		} else
			...
			if ($ret = parent::cleanVars($object))
			{
				foreach(array_keys($object->vars) as $field)
				{
					if ($object->vars[$field]['data_type'] == XOBJ_DTYPE_OTHER) {
						$object->vars[$field]['value'] = fontsvnCompressData($object->vars[$field]['value']);
					} elseif (strpos($field, 'pass')||strpos($field, 'password')) {
						$object->vars[$field]['value'] = fontsvnEncryptPassword($object->vars[$field]['value'], PLEASE_SALT . PLEASE_SALT_WHENSET);
					}
				}
			}
		return $ret;
 ...
 * @copyright copyright (c) 2015 labs.coop
 */
class fontsvnXoopsObjectHandler extends XoopsPersistableObjectHandler
{
