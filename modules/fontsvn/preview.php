 ...
 */
	...
	if (!$data = XoopsCache::read($cachekey = _MD_FONTSVN_MODULE_DIRNAME . "-preview-".md5($_GET['id'].$_GET['format'])) && !(isset($data['image']) && !empty($data['image'])))
	{
		// API Load Balancing
		...
		// Calls API
		$data = array('image'=>fontsvn_getURIData(str_replace("%apipath%", $fontsvnConfigsList['api_path'], str_replace("%identity%", $font->getVar('identity'), str_replace("%format%", $_GET['format'], $fontsvnConfigsList['api_path_preview'])))));
		if (!empty($data) && $fontsvnConfigsList['cache_preview'] > 0)
		{
			XoopsCache::write($cachekey, $data, $fontsvnConfigsList['cache_preview']);
			$caches = XoopsCache::read(_MD_FONTSVN_MODULE_DIRNAME . "-caches-session");
			if (!is_array($caches))
				$caches = array();
			$caches[$cachekey] = time() + $fontsvnConfigsList['cache_preview'];
