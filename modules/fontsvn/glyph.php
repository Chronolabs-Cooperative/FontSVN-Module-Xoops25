...
if (!isset($_GET['char']))
{
	redirect_header(XOOPS_URL . '/modules/'._MD_FONTSVN_MODULE_DIRNAME.'/index.php', 4, _ERR_CONVERT_GLYPH_NOCHARSPECIFIED);
	exit(0);
}
...
if (!$data = XoopsCache::read($cachekey = _MD_FONTSVN_MODULE_DIRNAME . "-glyph-".md5($_GET['id'].$_GET['char'].$_GET['format'])) && !(isset($data['image']) && !empty($data['image'])))
{
	// API Load Balancing
	...
	// Calls API
	$data = array('image'=>fontsvn_getURIData(str_replace("%apipath%", $fontsvnConfigsList['api_path'], str_replace("%identity%", $font->getVar('identity'), str_replace("%char%", $_GET['char'], str_replace("%format%", $_GET['format'], $fontsvnConfigsList['api_path_glyphs']))))));
	if (!empty($data) && $fontsvnConfigsList['cache_glyph'] > 0)
	{
		XoopsCache::write($cachekey, $data, $fontsvnConfigsList['cache_glyph']);
		$caches = XoopsCache::read(_MD_FONTSVN_MODULE_DIRNAME . "-caches-session");
		if (!is_array($caches))
			$caches = array();
		$caches[$cachekey] = time() + $fontsvnConfigsList['cache_glyph'];
