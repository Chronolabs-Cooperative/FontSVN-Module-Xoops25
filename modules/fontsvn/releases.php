 ...
 */
	...
	if ($GLOBALS['fontsvnConfigsList']['htaccess']) {
                if (!strpos(XOOPS_URL.'/'.$GLOBALS['fontsvnConfigsList']['base'].'/releases.html', $_SERVER['REQUEST_URI'])) {
                        header('Location: ' . XOOPS_URL.'/'.$GLOBALS['fontsvnConfigsList']['base'].'/releases.html');
                        exit(0);
                }
	}
	xoops_load('XoopsCache');
	if (!$data = XoopsCache::read($cachekey = _MD_FONTSVN_MODULE_DIRNAME . "-form-releases"))
	{
		// API Load Balancing
		...
		// Calls API
		$data = array('html'=>fontsvn_getURIData(str_replace("%apipath%", $fontsvnConfigsList['api_path'], $fontsvnConfigsList['api_path_releases_form']), array('return' => xoops_getModuleHandler('identities',_MD_FONTSVN_MODULE_DIRNAME)->getReleasesURL())));
	...
	$xoopsOption['template_main'] = 'fontsvn_releases.html';
	...
	$GLOBALS['xoopsTpl']->assign('xoops_pagetitle', _MN_FONTSVN_RELEASES_PAGETITLE);
